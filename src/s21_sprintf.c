  ...
  for (unsigned char* p = (unsigned char*)format; *p != '\0'; p++) {
    if (f.is_percent) {
      ...
      if (*p == 'd')
        ...
        f.wait_for_accuracy = 0;
        f.is_percent = 1;
        f.float_accuracy = *p - '0';
      } else if (*p == '+') {
        f.need_sign = 1;
        f.is_percent = 1;
      } else if (*p == ' ') {
        ...
        if (f.left_alignment == 0)
          f.left_alignment = *p - '0';
        else
          f.left_alignment = f.left_alignment * 10 + (*p - '0');
        f.is_percent = 1;
      }
    } else if (*p == '%')
      f.is_percent = 1;
...
int float_to_str(char* dest, double num, int accuracy, int need_sign,
  ...
  char* buffer =
      malloc(sizeof(char*) * 50);  // 20-21 цифра максимум, взял с запасом
  if (!buffer) {
    fprintf(stderr, "Error allocating memory");
    return 1;
  }
  ...
  if (is_negative && need_sign >= 0) {
    ...
    free(whole_str);  // освобождение whole_str, который указывает на buffer =>
                      // освобождаем buffer
    whole_str = tmp;
  }
  ...
  int fractional_int = (int)((fractional_part * multiplier) +
